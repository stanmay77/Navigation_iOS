...
class InfoViewController: UIViewController {
    ...
    let api1Label: UILabel = {
        let label = UILabel(frame: .zero)
        label.text = "Api1 title: "
    ...
    let api2Label: UILabel = {
        let label = UILabel(frame: .zero)
        label.text = "Api2 orbital period: "
        label.textColor = .red
        label.font = UIFont.systemFont(ofSize: 18, weight: .bold)
        label.translatesAutoresizingMaskIntoConstraints = false
        label.numberOfLines = 0
    ...
    func configureUI() {
        ...
        view.addSubview(api1Label)
        view.addSubview(api2Label)
        
        NSLayoutConstraint.activate([
            api1Label.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 20),
            api1Label.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            api1Label.widthAnchor.constraint(equalToConstant: 300),
            api2Label.topAnchor.constraint(equalTo: api1Label.bottomAnchor, constant: 50),
            api2Label.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            api2Label.widthAnchor.constraint(equalToConstant: 300)
        ])
    }
    
    func getDataApi1() {
        NetworkManager.shared.requestSerialization(for: URL(string: "https://jsonplaceholder.typicode.com/todos/1")!) {[weak self] result in
            ...
            case .success(let res):
                if res!["title"] != nil {
                    DispatchQueue.main.async {
                        self!.api1Label.text! += res!["title"]! as! String
                    }
                }
                    else {
                        DispatchQueue.main.async {
                            self!.api1Label.text = "No title received"
                        }
                    }
                }
            }
...
func getDataApi2() {
    NetworkManager.shared.requestDecode(at: URL(string:"https://swapi.dev/api/planets/1")!, for: apiObject2.self) {[weak self] result in
        ...
        case .success(let res):
                    DispatchQueue.main.async {
                        self!.api2Label.text! += res.orbitalPeriod
                    }
                }
            }
    }
